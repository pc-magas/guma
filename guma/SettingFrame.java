...
public class SettingFrame extends JPanel implements ItemListener
{
	...
	*/
	private JSpinner pli8osPraxewn=new JSpinner(new SpinnerNumberModel());
	...
	*/
	private JSpinner maxNum=new JSpinner(new SpinnerNumberModel());		
	
	/**
	*JLabel that describes what we set with JSPinner maxNum
	*/
	private JLabel maxNumLabel=new JLabel("Μεγιστη τιμή όπου θα είναι ως όρος σε μια πράξη:");
	...
	*/
	...
	/**
	*Returns the game that the frame crated
	*/	
	...
	{	String[] options={"OK","Cancel"};
		...
		if(returnVal==JOptionPane.OK_OPTION)
		{
			...
			int pli8os=pli8osModel.getNumber().intValue();
			int maximum=maxNumModel.getNumber().intValue();
			
			if(praxeis.isEmpty())
			{
				JOptionPane.showMessageDialog((Component)this,"Δεν επιλέξατε πράξη.\n"
								+"Παρακαλώ επαναλάβεται την διαδικασία και"+
								" επιλέξτε πράξη.",
								"Μου άλαξες τα φώτα",
								JOptionPane.ERROR_MESSAGE);
			}
			if(pli8os>0&&maximum>0)
			{
				gameToCreate=new Game(pli8os,maximum,p2);
			}
			else
			{
				JOptionPane.showMessageDialog((Component)this,"Λάθος τιμές στα πεδία:\n"+
								"Παρακαλώ επαναλάβεται την διαδικασία\n και "
								+"εισάγεται θετικές μη μηδενικές"+
								" τιμές στα πεδία\n"+
								"\"Επιλέξτε είδος πράξεων\" και\n"+
								"\"Μεγιστη τιμή όπου θα είναι ως όρος σε μια "+ 									"πράξη:\"",
								"Μου άλαξες τα φώτα",
								JOptionPane.ERROR_MESSAGE);
			}
